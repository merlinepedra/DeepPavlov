...
class QueryGenerator(QueryGeneratorBase):
    ...
    def __call__(self, question_batch: List[str],
                 ...
                 answer_type_str_batch: List[str] = None) -> List[Union[List[Tuple[str, Any]], List[str]]]:
        ...
        if answer_type_str_batch is None:
            answer_type_str_batch = ["" for _ in question_batch]
        log.info(f"kbqa inputs {question_batch} {entities_from_ner_batch}")
        for question, question_sanitized, template_type, entities_from_ner, types_from_ner, q_type_flag, answer_type_str in \
                zip(question_batch, question_san_batch, template_type_batch,
                    entities_from_ner_batch, types_from_ner_batch, q_type_flags_batch, answer_type_str_batch):
            if template_type == "-1":
                template_type = "7"
            candidate_outputs, template_answer, templates_nums = self.find_candidate_answers(question, question_sanitized,
        ...
        if self.return_answers:
            answers = self.rel_ranker(question_batch, candidate_outputs_batch, entities_from_ner_batch,
                                      template_answers_batch, answer_type_str_batch, templates_nums_batch, q_type_flags_batch)
