...
class ParagraphRanking(Component):
    ...
    def __call__(self, questions):
        ...
        tm_end = time.time()
        print(f"exec time: {tm_end - tm_st}")
        return doc_titles_batch


ranker = ParagraphRanking(pretrained_bert="DeepPavlov/rubert-base-cased",
                          embedder_weights_path="/archive/evseev/.deeppavlov/models/paragraph_bienc_ranking_long_rus/question_encoder.pth.tar",
                          faiss_index_filename="/archive/evseev/RuWiki/wiki_archives/wikipedia_faiss_long/vectors.index",
                          id2doc_title_filename="/archive/evseev/.deeppavlov/downloads/paragraph_ranking/doc_titles2.pickle",
                          num_cand_par=1000000
                         )

conn = sqlite3.connect("/archive/evseev/.deeppavlov/downloads/paragraph_ranking/wikipedia_rus2.db")

c = conn.cursor()

res = ranker(["Где в мире самое крупное животное?"])
#out = open("log_found_doc_titles.txt", 'w')

all_doc_titles = {elem: n for n, elem in enumerate(res[0])}

inp = open("/home/evseev/EL_ranking/DeepPavlov/log_tfidf_doc_titles.txt", 'r')
found_tfidf_titles = inp.readlines()
found_tfidf_titles = [title.strip() for title in found_tfidf_titles]

out = open("tfidf_bert_scores.txt", 'a')
for title in found_tfidf_titles:
    out.write(f"{title} {all_doc_titles.get(title, -1)}"+'\n')

#for doc_title in res[0]:
#    res = c.execute('''SELECT text FROM documents WHERE id="{}";'''.format(doc_title))
#    out.write(str(doc_title)+'\n')
#    out.write(str(res.fetchall())+'\n')
#    out.write("_"*70+'\n')
