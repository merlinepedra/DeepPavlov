...
class BertBiencRankerLong(TorchModel):
    ...
    def train_on_batch(self, q_features: List[Dict],
        ...
        for elem in ['input_ids', 'attention_mask', 'token_type_ids']:
            ...
            _input[f"q_{elem}"] = torch.LongTensor(inp_elem).to("cuda:0")
        for elem in ['input_ids', 'attention_mask', 'token_type_ids']:
            inp_elem = [f[elem] for c_features in c_features_list for f in c_features]
            _input[f"c_{elem}"] = torch.LongTensor(inp_elem).to("cuda:0")
...
class BiencoderLong(nn.Module):
    ...
    def forward(
        ...
        #print("q_input_ids", len(q_input_ids), "c_input_ids", len(c_input_ids))
...
class BertBiencRankingLong(nn.Module):
    ...
    def __init__(
        ...
        self.biencoder.load()
        self.biencoder = torch.nn.DataParallel(self.biencoder, device_ids=[0, 2, 3, 4, 5])
        self.biencoder.to("cuda:0")
    ...
    def forward(
        ...
        #print("input q_length", len(q_input_ids), "c_length", len(c_input_ids))
        ...
        if positive_idx is not None:
            ...
            loss = F.nll_loss(softmax_scores, torch.tensor(positive_idx).to("cuda:0"), reduction="mean")
